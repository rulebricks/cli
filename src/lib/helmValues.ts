 ...
 * deep-merged over the existing values file, then exact CLI-owned conditional
 * paths are replaced or removed. Manual values.yaml edits outside those paths
 * survive while mode transitions cannot retain stale generated values. In k8s
 * secret mode the result is re-redacted so inline secrets carried over from
 * older values files never survive the merge. Falls back to plain generation
 * when no values file exists yet.
 */
...
export function mergeHelmValues(
  ...
  let result = deepMerge(existing, updates);

  // Most generated values are safely overlaid: that preserves manual chart
  // tuning below the same parent objects. These paths are different. They are
  // exclusive, config-selected shapes where a missing key means "turn this
  // mode off", not "keep the value from the previous generation".
  //
  // Keep the paths narrow. In particular, do not replace broad trees such as
  // rulebricks, supabase, vector, or kafka: customers can add chart-supported
  // values there which generation does not know about.
  for (const path of CLI_OWNED_EXCLUSIVE_PATHS) {
    result = reconcileOwnedPath(result, updates, path);
  }
  result = reconcileOwnedNamedArrayItem(
    result,
    existing,
    updates,
    ["vector", "extraContainers"],
    "kafka-proxy",
  );
...
type HelmValuePath = readonly string[];

const CLI_OWNED_EXCLUSIVE_PATHS: readonly HelmValuePath[] = [
  // Email mode: smtpRelay only exists for ACS API-key delivery.
  ["global", "smtp"],
  ["smtpRelay"],

  // Optional global feature blocks must lose credentials/provider-specific
  // fields when disabled or switched.
  ["global", "sso"],
  ["global", "tracing"],
  ["traefik", "tracing"],
  ...
  ...[
    ...
    "newrelic",
    "axiom",
  ].map((name) => ["vector", "customConfig", "sinks", name] as const),
  ...["elasticsearch", "loki", "generic_http"].map(
    (name) => ["vector-agent", "customConfig", "sinks", name] as const,
  ),

  // Optional TLS material is absent when the deployment returns to chart-managed
  // public certificates.
  ["global", "tlsIssuerRef"],
  ["global", "tlsPrivateCaBundle"],

  // External Redis is a mutually exclusive shape. Leave the embedded
  // persistence tree mergeable so customer sizing fields still survive.
  ["rulebricks", "redis", "enabled"],
  ["rulebricks", "redis", "external"],

  // Managed Postgres-only seams. DB_SSL is owned one leaf at a time so custom
  // service environment variables remain intact when returning to embedded DB.
  ["supabase", "externalDatabase"],
  ["migrations", "externalDb"],
  ["supabase", "auth", "environment", "DB_SSL"],
  ["supabase", "rest", "environment", "DB_SSL"],
  ["supabase", "realtime", "environment", "DB_SSL"],
  ["supabase", "meta", "environment", "DB_SSL"],

  // Kafka connection fields switch between embedded auto-discovery, direct
  // SASL, and the AWS IAM proxy bridge. Own only those fields, not the entire
  // app logging or Vector source trees.
  ["rulebricks", "app", "logging", "enabled"],
  ["rulebricks", "app", "logging", "kafkaBrokers"],
  ["rulebricks", "app", "logging", "kafkaTopic"],
  ["rulebricks", "app", "logging", "kafkaTopicPrefix"],
  ["rulebricks", "app", "logging", "kafkaSsl"],
  ["rulebricks", "app", "logging", "kafkaSasl"],
  ["vector", "customConfig", "sources", "kafka", "sasl"],
  ["kafkaBridge"],
];
...
/**
 * Replaces an exact CLI-owned path with the generated value, or deletes it when
 * generation omitted it. Branches are cloned on the way down so reconciliation
 * never mutates the caller's existing values object.
 */
function reconcileOwnedPath(
  merged: Record<string, unknown>,
  generated: Partial<Record<string, unknown>>,
  path: HelmValuePath,
): Record<string, unknown> {
  const generatedEntry = valueAtPath(generated, path);
  const mergedEntry = valueAtPath(merged, path);
  if (!generatedEntry.found && !mergedEntry.found) return merged;

  return updatePath(
    merged,
    path,
    0,
    generatedEntry.found,
    generatedEntry.value,
  );
}

/**
 * Replace/remove one CLI-owned member of a chart array while preserving
 * customer-defined siblings. Arrays cannot use deep path reconciliation
 * because generated arrays replace the existing value during deepMerge.
 */
function reconcileOwnedNamedArrayItem(
  merged: Record<string, unknown>,
  ...
  const existingEntry = valueAtPath(existing, path);
  const generatedEntry = valueAtPath(generated, path);
  const existingItems = Array.isArray(existingEntry.value)
    ? existingEntry.value
    : [];
  const generatedItems = Array.isArray(generatedEntry.value)
    ? generatedEntry.value
    : [];
  const generatedOwned = generatedItems.find(
    (item) => isRecord(item) && item.name === ownedName,
  );

  let replaced = false;
  const reconciled = existingItems.flatMap((item) => {
    if (!isRecord(item) || item.name !== ownedName) return [item];
    if (replaced || !generatedOwned) return [];
    replaced = true;
    return [generatedOwned];
  });
  if (generatedOwned && !replaced) reconciled.push(generatedOwned);
...
function updatePath(
  ...
  result[key] = updatePath(child, path, index + 1, shouldSet, value);
  return result;
}
