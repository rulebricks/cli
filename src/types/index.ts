...
export const DeploymentConfigSchema = z.object({
  ...
  // Raw decision logs, native ClickHouse parts, and DB backups use separate
  // key prefixes within it.
  storage: z
    .object({
      ...
      paths: z
        .object({
          ...
          clickhouse: z.string().min(1).optional(),
  ...
  // Persistent mode keeps native MergeTree objects in shared object storage
  // with separate metadata and cache PVCs; stateless mode queries the raw archive.
  clickhouse: z
    .object({
      persistence: z
        .object({
 ...
 * Keep the historical 20-character cap so upgrades remain compatible with
 * resource names generated by earlier chart versions.
 */
